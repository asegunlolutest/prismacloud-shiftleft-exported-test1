...
# Scan image
./twistcli images scan "$ACR_FQDN/$IMAGE_NAME" --address $PC_CONSOLE -u $PCUSER -p $PCPASS
...
if [ "$scanresult" == $BUILD_IMAGE_SCAN_FLAG ]; then
   ...
   ./twistcli images scan "$ACR_FQDN/$IMAGE_NAME" --address $PC_CONSOLE -u $PCUSER -p $PCPASS --details
   exit 1
fi
